...
**Status: Phase 9 BALANCE PASS IN PROGRESS — harness DONE, tuning round 1 DONE (gun/hack mono-cruises broken, per-run RNG reseed); awaiting user browser playtest before further tuning (economy untouched by design)** (updated 2026-07-09)
...
- [x] **Scenario harness first** (user request 2026-07-07) — **DONE 2026-07-09**: `npm run scenarios` runs 11 scenarios in ~6s (baseline, no-upgrades, no-interception, 8 mono-tower runs) and prints the comparison table + economy probe. Shared auto-player machinery extracted to `scripts/lib/autoplayer.ts` (parameterized `Strategy`); `smoke.ts` now reuses it, behavior unchanged. **Findings**: mono-GUN cruises to w50 6/6 and mono-HACK to w50 4/6 (degenerate solo strategies — first tuning targets); economy runs away after w37 (end-of-round cash $1k→$30.5k by w50); no-interception dies w18; no-upgrades dies w38; mono-repulsor stalls at w4 (pure control can't clear a round — expected). Note: mono-DRONE dies at w14, so the user's "T3 drones OP" feel is about drone mass *within a mixed build*, not solo — needs a targeted scenario or playtest before drone tuning. Original harness spec:
...
- [x] Interview the user for felt playtest targets (what's boring, what's mandatory, what never gets bought) before changing numbers — **done 2026-07-09, findings:**
  ...
  - **Mono-gun target (user, 2026-07-09):** don't make it impossible — "a bajillion guns" should stay competitive-but-not-guaranteed, able to limp near wave 50 with luck while bleeding cores. Tuning target is breaking the comfortable 6/6 cruise, not killing the strategy.
...
- [ ] Tune economy (income/bounties/costs together), stronger-enemy volume as stages progress, and per-tower numbers across all ten towers — all in `src/balance.ts`/`src/content/`, re-running scenarios after each change
  - **Tuning round 1 (2026-07-09, DONE):** gun T2/T3 DPS 16/24 → 10/12 (T1 untouched; breakpoint-scanned — T3=14 still cruised, 12 is the gentlest effective value) and hack cooldowns 9/7/5.5 → 11/9/7 (per-conversion punch untouched). Result: mono-gun now dies at w49 losing all cores in the final stretch (exactly the user's "limps close, not guaranteed" target), mono-hack dies w15, baseline still clears w50. **Harness fix (senior):** `seedRng()` added to `src/sim/rng.ts` and called per `runAutoPlay` — scenarios previously shared one RNG stream, so any tuning change reshuffled every later scenario's enemies; results are now order-independent.
  - **Drone verdict (drone-heavy mixed scenario, 2026-07-09):** 18 drones on a gun/flak spine dies w47 — far beyond mono-drone (w14) but *below* the winning baseline. The harness shows drones viable-but-not-dominant; no drone nerf without contrary playtest evidence.
...
- ~~**Build bar buries the map on small screens** (user playtest 2026-07-07, iPhone SE simulator)~~ **FIXED 2026-07-07**: the ten-slot build bar is now collapsible via a ▲/▼ BUILD toggle, auto-hides while a placement is in progress, and a red ✕ CANCEL <tower> button stands in as the touch equivalent of ESC/right-click.
- ~~**Tower/core panel collides with bottom controls on phones** (user playtest 2026-07-07, iPhone SE simulator)~~ **FIXED 2026-07-07**: on screens ≤640px wide the panel moves to the top and pops over the radar — the one area that isn't map or controls.
- ~~**Battery model showed twin silos at every tier** (user, 2026-07-07)~~ **FIXED 2026-07-07**: the second silo tube was decorative and contradicted the data (silos: 2 is the T3 upgrade). It's now hidden until T3 — the first live instance of the tier-visuals-show-capability principle from the backlog.
- ~~**EXODEF COMMAND mark obscured by the unit picker** (user playtest 2026-07-07)~~ **FIXED 2026-07-07**: the arcade mark is pinned top-left under the cash readout with trimmed hints (1-0 build · TAB intercept · ENTER start); on phone widths only the mark shows.
- ~~**Speed/config/start controls shifted around as the picker/labels changed size** (user playtest 2026-07-07)~~ **FIXED 2026-07-07**: the bottom bar is now two pinned lines — the unit picker always opens on its own line above the control row, which stays anchored to the bottom edge regardless of picker state or button text width.
- ~~**Placing a unit auto-opened its info panel and trapped it open** (user playtest 2026-07-07)~~ **FIXED 2026-07-07**: placement mode and the info panel are now mutually exclusive — placing no longer auto-selects the new tower, and entering placement mode closes any open panel. The panel opens only by clicking a tower/core outside placement mode.
...
- **Visible upgrade status** (user, 2026-07-07, to workshop later): towers should visually show their tier at a glance — currently T1/T2/T3 look identical on the map and only the panel reveals it. **The drone tower is the exemplar** (user): its hovering drone count IS the tier readout, and it's functional rather than decorative — prefer tier visuals that show the actual capability where possible (e.g., battery silo count already scales at T3). Beyond that, the tower models themselves could change with upgrades (model growth, added greebles per tier, tier pips/glow, color shift — pick in a design chat, keep flat-shaded readability).
...
- **2026-07-09 (tuning round 1)** — Opus agent tuned gun (T2/T3 DPS 16/24→10/12) and hack (cooldowns 9/7/5.5→11/9/7); both mono-cruises broken while baseline still clears w50 6/6 and smoke passes. Mono-gun hits the user's revised target: limps to w49, then loses all six cores. New "drone-heavy mixed" scenario dies w47 — drones-in-mix strong but not dominant, no nerf. Agent surfaced a real harness flaw: the module-global RNG stream (`src/sim/rng.ts`) was shared across scenarios, so any tuning change reshuffled later scenarios' enemy sequences (mono-hack's original w50 4/6 was partly RNG phase). Senior fix: `seedRng()` export + per-`runAutoPlay` reseed; suite is now order-independent. Economy deliberately untouched (user deferred to playtest; auto-player's finite queue overstates late surplus). Next: user browser playtest of the new gun/hack feel decides round 2.
- **2026-07-09 (scenario harness)** — Phase 9 harness landed (Opus agent build, senior-reviewed): `scripts/lib/autoplayer.ts` (shared parameterized auto-player, extracted from smoke), `scripts/scenarios.ts` (`npm run scenarios`, 11 scenarios ~6s), smoke refactored onto the shared module with identical behavior. Review fix: smoke re-throws on a sim stall (the extracted runner records stalls as results, which would have let a wave-50 stall pass smoke silently). Headline data: mono-gun w50 6/6, mono-hack w50 4/6, economy runaway after w37 ($30.5k idle by w50, $226k total end-of-round surplus), no-interception dies w18. Mono-drone dies w14 — drone OP-ness is a mixed-build/late-game effect, not solo. `npm run build` + `npm run smoke` + `npm run scenarios` all pass. Next: tuning, re-running scenarios after each change.
